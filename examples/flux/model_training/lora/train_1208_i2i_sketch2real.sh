accelerate launch --config_file examples/flux/model_training/full/accelerate_config.yaml examples/flux/model_training/train.py \
  --dataset_base_path /fi-lib/workspace/sjx/DiffSynth-Studio/ \
  --dataset_metadata_path dataset/datasets/sketch2real.txt \
  ...
  --output_path "./models/train/FLUX.1_lora_1208_i2i_fixcaption" \
  --lora_base_model "dit" \
  ...
  --default_caption "convert this sketch into real film photos, each photo is consists of 6 small pictures" > log/flux_train_1208_i2i_sketch2real.log 2>&1